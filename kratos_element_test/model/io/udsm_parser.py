...
def get_model_name(getmodelname, model_no):
    ...
    getmodelname(
        ctypes.byref(ctypes.c_int(model_no)), char_buffer, ctypes.c_long(BUFFER_SIZE)
    )
    return clean_c_buffer(char_buffer)


def get_param_count(getparamcount, model_no):
    getparamcount.argtypes = (
        ctypes.POINTER(ctypes.c_int),
        ctypes.POINTER(ctypes.c_int),
    )
...
def get_param_name(getparamname, model_no, param_no):
    ...
    getparamname(
        ctypes.byref(ctypes.c_int(model_no)),
        ctypes.byref(ctypes.c_int(param_no)),
        char_buffer,
        ctypes.c_long(BUFFER_SIZE),
    )
    return clean_c_buffer(char_buffer)
...
def udsm_parser(dll_path):
    ...
    for model_no in range(1, model_count + 1):
        ...
        param_names = [
            input_parameters_format_to_unicode(
                get_param_name(getparamname, model_no, p)
            )
